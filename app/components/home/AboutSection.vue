 ...
 * paragraph block on the right. Hydrated from ContentBlocks (`about_intro`
 * and an optional `about_intro_2`); falls back to i18n.
 */
...
const blockText = (key: string) => props.contentBlocks.find((b) => b.key === key)?.text;
const blockKind = (key: string) => props.contentBlocks.find((b) => b.key === key)?.kind;
...
<template>
  <section>
    <div class="max-w-[1280px] mx-auto px-6 py-16 grid grid-cols-1 md:grid-cols-12 gap-6">
      ...
      <div
        class="md:col-span-9 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 text-[16px] sm:text-[16.5px] leading-[1.7] text-ink-2 fade-up"
      >
        <!-- First column -->
        <div v-if="intro1Kind === 'html' && intro1" v-html="intro1" />
        <p v-else-if="intro1">{{ intro1 }}</p>
        <p v-else>{{ $t('home.about.fallback1') }}</p>

        <!-- Second column -->
        <div v-if="intro2Kind === 'html' && intro2" v-html="intro2" />
        <p v-else-if="intro2">{{ intro2 }}</p>
        <p v-else>{{ $t('home.about.fallback2') }}</p>
      </div>
    </div>
  </section>
</template>
